...
export default function RecipesList() {
  ...
  return (
    <div className="space-y-6">
      {/* ... ZACHOWUJEMY STRUKTURĘ JSX BEZ ZMIAN ... */}
              ...
              <li key={r.id} className="bg-card border border-primary dark:border-primary rounded-2xl shadow-lg p-5 animate-in fade-in">
                <div className="space-y-4">
                  <div>
                    ...
                    <input ref={nameRef} type="text" value={editedRecipe.name} onChange={(e) => setEditedRecipe({ ...editedRecipe, name: e.target.value })} className="input-field font-medium" />
                  </div>
                  <div>
                    <label className="form-label">Kategoria:</label>
                    <select value={editedRecipe.category || "śniadanie"} onChange={(e) => setEditedRecipe({ ...editedRecipe, category: e.target.value as RecipeCategory })} className="input-field py-1.5">
                      {CATEGORIES.map((c) => (
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      <input value={prodInput} onChange={(e) => setProdInput(e.target.value)} onKeyDown={onProdKeyDown} placeholder="Dodaj składnik..." className="input-field flex-1" />
                      <button type="button" onClick={() => commitProduct(prodInput)} className="px-4 bg-primary text-white hover:bg-secondary rounded-xl transition-colors shadow-sm shrink-0" title="Dodaj składnik">
                    ...
                    {suggestions.length > 0 && (
                      <div className="mt-2 rounded-xl border border-gray-200 dark:border-gray-700 bg-surface shadow-lg max-h-40 overflow-y-auto divide-y divide-gray-100 dark:divide-gray-800 custom-scrollbar">
                        {suggestions.map((s) => (
                          <button key={s} type="button" onClick={() => commitProduct(s)} className="w-full text-left px-4 py-2 hover:bg-card text-sm font-medium transition-colors">{s}</button>
                        ))}
                      </div>
                    )}
                  ...
                  <div>
                    ...
                    <textarea value={editedRecipe.description || ""} onChange={(e) => setEditedRecipe({ ...editedRecipe, description: e.target.value })} className="input-field" rows={5} placeholder="Krok po kroku..." />
                  </div>
