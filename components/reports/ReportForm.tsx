...
export default function ReportForm({ onChange, onCancel }: ReportFormProps) {
  ...
  const removeItem = (arr: any[], setter: any, index: number) => {
    if (arr.length > 1) setter(arr.filter((_, i) => i !== index));
  };
    ...
    <form onSubmit={handleSubmit} className="form-card max-w-3xl">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <input
            ...
            required
            disabled={loading}
            placeholder="np. Tygodniowy status"
          />
        </div>
        <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
            ...
            {agenda.map((a, i) => (
              <div key={i} className="flex gap-2">
                <input
                  ...
                  value={a}
                  onChange={(e) => {
                    const copy = [...agenda]; copy[i] = e.target.value; setAgenda(copy);
                  }}
                  placeholder={`Punkt agendy ${i + 1}`}
                  disabled={loading}
                />
                {agenda.length > 1 && (
          ...
          <button type="button" onClick={() => setAgenda([...agenda, ""])} className="text-sm font-medium text-primary hover:text-secondary mt-2 flex items-center">
        ...
        <div>
          ...
          <button type="button" onClick={() => setParticipants([...participants, ""])} className="text-sm font-medium text-primary hover:text-secondary mt-2 flex items-center">
      ...
      <div className="pt-4 border-t border-gray-100 dark:border-gray-800">
        ...
        <div className="space-y-3">
            ...
            <div key={i} className="p-3 border border-gray-200 dark:border-gray-700 rounded-lg bg-surface space-y-3">
              <div className="flex gap-2">
                <input
                  ...
                  value={t.zadanie}
                  onChange={(e) => {
                    const copy = [...tasks]; copy[i].zadanie = e.target.value; setTasks(copy);
                  }}
                  disabled={loading}
                />
                {tasks.length > 1 && (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <input
                  type="date"
                  className="input-field bg-card w-full min-w-0 px-1 text-xs"
                  value={t.data}
                  onChange={(e) => {
                ...
                <input
                  ...
                  value={t.osoba}
                  onChange={(e) => {
                    const copy = [...tasks]; copy[i].osoba = e.target.value; setTasks(copy);
                  }}
                  disabled={loading}
                />
              </div>
          ...
          <button type="button" onClick={() => setTasks([...tasks, { zadanie: "", data: "", osoba: "" }])} className="text-sm font-medium text-primary hover:text-secondary flex items-center mt-2">
      ...
      <div>
        ...
        <textarea
          ...
          className="input-field"
          rows={5}
          placeholder="Podsumowanie, wnioski..."
          disabled={loading}
        />
      </div>
