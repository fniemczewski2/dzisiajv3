...
return (
    <div className="widget flex items-center justify-between px-4 py-3">
      ...
      <div className="flex flex-nowrap gap-1 md:gap-2">
        {options.map(opt => {
          // NAPRAWA 1: Sprawdzamy, czy ten konkretny przycisk jest wybranym nastrojem
          const isSelected = todayMood?.mood_id === opt.id;

          return (
            <button 
              key={opt.id}
              onClick={() => logMood(date, opt.id)}
              className={`group px-2.5 py-1 md:px-3 md:py-1.5 flex items-center flex-1 h-[30px] rounded-full text-xs font-semibold text-white shadow-sm transition hover:scale-105 active:scale-95
                ${isSelected ? 'opacity-100 scale-120' : 'opacity-60 hover:opacity-80'}`}
              style={{ backgroundColor: opt.color }}
              title={opt.label}
            >
              {/* NAPRAWA 2: Poprawne klasy Tailwind dla ukrywania/pokazywania elementu */}
              {isSelected && <span> <Check className="w-4 h-4 mt-0.5"/> </span>}
