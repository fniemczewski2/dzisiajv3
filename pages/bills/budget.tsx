...
export default function BudgetPage() {
  ...
  return (
    <>
      <Head><title>Budżet – {year}</title></Head>
      <Layout>
        <div className="flex justify-between gap-3 items-center mb-4">
          <span className="flex sm:flex-1 flex-nowrap">
            <button onClick={handleBack} className="p-2 flex items-center bg-primary hover:bg-secondary text-white rounded-lg shadow" aria-label="Wróć">
              <ChevronLeft className="w-4 h-4" />
            </button>
            <h2 className="text-xl ml-2 font-semibold">Statystyki</h2>
          </span>
          <div className="items-center sm:flex-1 justify-center flex">
            <button onClick={() => setYear((y) => y - 1)} className="p-2 bg-card rounded-lg hover:bg-surface" aria-label="Poprzedni rok">
              <ChevronLeft className="w-4 h-4" />
            </button>
            <h2 className="text-lg font-semibold mx-4">{year}</h2>
            <button onClick={() => setYear((y) => y + 1)} className="p-2 bg-card rounded-lg hover:bg-surface" aria-label="Następny rok">
