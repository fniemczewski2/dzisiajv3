...
export default function DashboardPage({isMain, date = todayDate}: DashboardPageProps) {
  const { user } = useAuth();
  ...
  const planByHour = useMemo(() => {
    ...
    Object.keys(map).forEach((timeKey) => {
      ...
      if (isPast) {
        const hasUnfinishedTask = map[timeKey].some((item) => item.type === "task");
